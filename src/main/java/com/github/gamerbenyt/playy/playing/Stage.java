...
public class Stage {
	private final MinecraftClient MC = Playy.MC;
	...
	public void sendMovementPacketToStagePosition() {
		...
		Playy.MC.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(
				position.getX() + 0.5, position.getY(), position.getZ() + 0.5,
				Playy.MC.player.getYaw(), Playy.MC.player.getPitch(),
				true, false));
	}
	...
	public void checkBuildStatus(Song song) {
		...
		for (BlockPos nbPos : noteblockLocations) {
			BlockState bs = Playy.MC.world.getBlockState(nbPos);
			int blockId = Block.getRawIdFromState(bs);
			if (blockId >= Playy.NOTEBLOCK_BASE_ID && blockId < Playy.NOTEBLOCK_BASE_ID+800) {
				int noteId = (blockId-Playy.NOTEBLOCK_BASE_ID)/2;
	...
	// This code was taken from Sk8kman fork of Playy
		...
		for (int dx = -5; dx <= 5; dx++) {
			for (int dz = -5; dz <= 5; dz++) {
				for (int dy : new int[]{-1, 0, 1, 2, -2, 3, -3, 4, -4, 5, 6}) {
					...
					BlockState bs = Playy.MC.world.getBlockState(bp);
					BlockState aboveBs = Playy.MC.world.getBlockState(bp.up());
					int blockId = Block.getRawIdFromState(bs);
					if (blockId >= Playy.NOTEBLOCK_BASE_ID && blockId < Playy.NOTEBLOCK_BASE_ID + 800 && aboveBs.isAir()) {
						int noteId = (blockId - Playy.NOTEBLOCK_BASE_ID) / 2;
	...
	public boolean hasBreakingModification() {
		for (Map.Entry<Integer, BlockPos> entry : noteblockPositions.entrySet()) {
			BlockState bs = Playy.MC.world.getBlockState(entry.getValue());
			int blockId = Block.getRawIdFromState(bs);
			int actualNoteId = (blockId-Playy.NOTEBLOCK_BASE_ID)/2;
			...
			BlockState aboveBs = Playy.MC.world.getBlockState(entry.getValue().up());
