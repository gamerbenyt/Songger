...
import com.github.gamerbenyt.playy.Config;
import com.github.gamerbenyt.playy.FakePlayerEntity;
import com.github.gamerbenyt.playy.Playy;
import com.github.gamerbenyt.playy.Util;
import com.github.gamerbenyt.playy.mixin.ClientPlayerInteractionManagerAccessor;
import com.github.gamerbenyt.playy.song.*;
...
public class SongHandler {
    ...
    public void onUpdate(boolean tick) {
        ...
        if (cleaningUp || currentSong != null) {
            ...
            if (fakePlayer != null) {
                fakePlayer.getInventory().clone(Playy.MC.player.getInventory());
            }

            // Maintain flying status
            wasFlying = Playy.MC.player.getAbilities().flying;
        }
        ...
        // Otherwise, handle cleanup if necessary
            ...
            if (dirty) {
                ...
                originalGamemode = Playy.MC.interactionManager.getCurrentGameMode();
            }
        }
    }
    ...
    private void handleBuilding() {
        ...
        if (!Config.getConfig().survivalOnly && Playy.MC.interactionManager.getCurrentGameMode() != GameMode.CREATIVE) {
            return;
        }
        ...
        if (!Config.getConfig().survivalOnly) { // Regular mode
            if (!stage.requiredBreaks.isEmpty()) {
                ...
                while (consumePlaceAllowance()) {
                    ...
                    int currentNoteId = (blockId - Playy.NOTEBLOCK_BASE_ID) / 2;
            ...
            if (!stage.requiredClicks.isEmpty()) {
                ...
                if (Playy.MC.world.getBlockState(bp).getBlock() == Blocks.NOTE_BLOCK) {
                    placeBlock(bp);
                }
                buildEndDelay = 20;
            }
    ...
    private void handlePlaying(boolean tick) {
        ...
        if (Playy.MC.interactionManager.getCurrentGameMode() != GameMode.SURVIVAL) {
            currentSong.pause();
            return;
        }
    ...
    private void handleCleanup() {
        ...
        ClientWorld world = Playy.MC.world;
        if (Playy.MC.interactionManager.getCurrentGameMode() != GameMode.CREATIVE) {
            return;
        }
    ...
    private void checkCleanupStatus() {
        ...
        cleanupBreakList = cleanupBreakList.stream()
                .sorted((a, b) -> {
                    ...
                    boolean a_grav = Playy.MC.world.getBlockState(a).getBlock() instanceof FallingBlock;
                    boolean b_grav = Playy.MC.world.getBlockState(b).getBlock() instanceof FallingBlock;
                    if (a_grav && !b_grav) {
                        return 1;
    ...
    public void restoreStateAndReset(boolean returnToStage) {
        ...
        if (originalGamemode != Playy.MC.interactionManager.getCurrentGameMode() && !Config.getConfig().survivalOnly) {
        ...
        if (Playy.MC.player.getAbilities().allowFlying == false) {
            Playy.MC.player.getAbilities().flying = false;
        }
    ...
    private void holdNoteblock(int id, int slot) {
        PlayerInventory inventory = Playy.MC.player.getInventory();
        inventory.setSelectedSlot(slot);
        ((ClientPlayerInteractionManagerAccessor) Playy.MC.interactionManager).invokeSyncSelectedSlot();
        ...
        Playy.MC.interactionManager.clickCreativeStack(noteblockStack, 36 + slot);
    }
    private void holdBlock(BlockState bs, int slot) {
        ...
        Playy.MC.interactionManager.clickCreativeStack(stack, 36 + slot);
    }
    private void placeBlock(BlockPos bp) {
        ...
        Playy.MC.interactionManager.interactBlock(Playy.MC.player, Hand.MAIN_HAND, new BlockHitResult(new Vec3d(fx, fy, fz), Direction.UP, bp, false));
        doSwingIfNeeded();
    }
    private void attackBlock(BlockPos bp) {
        doRotateIfNeeded(bp.getX() + 0.5, bp.getY() + 0.5, bp.getZ() + 0.5);
        Playy.MC.interactionManager.attackBlock(bp, Direction.UP);
        doSwingIfNeeded();
    }
    private void stopAttack() {
        Playy.MC.interactionManager.cancelBlockBreaking();
    }
    private void recordBlocks(Iterable<BlockPos> bpList) {
        for (BlockPos bp : bpList) {
            if (!originalBlocks.containsKey(bp)) {
                BlockState bs = Playy.MC.world.getBlockState(bp);
    ...
    private void doRotateIfNeeded(double lookX, double lookY, double lookZ) {
        if (Config.getConfig().rotate) {
            ...
            double e = lookY - (lastStage.position.getY() + Playy.MC.player.getStandingEyeHeight());
            ...
            Playy.MC.player.networkHandler.getConnection().send(new PlayerMoveC2SPacket.Full(
    ...
    private void doSwingIfNeeded() {
        if (Config.getConfig().swing) {
            Playy.MC.player.swingHand(Hand.MAIN_HAND);
            if (fakePlayer != null) {
    ...
    private void getAndSaveBuildSlot() {
        buildSlot = Playy.MC.player.getInventory().getSwappableHotbarSlot();
        prevHeldItem = Playy.MC.player.getInventory().getStack(buildSlot);
    }
    private void restoreBuildSlot() {
        if (buildSlot != -1) {
            Playy.MC.player.getInventory().setStack(buildSlot, prevHeldItem);
            Playy.MC.interactionManager.clickCreativeStack(prevHeldItem, 36 + buildSlot);
            buildSlot = -1;
        }
    }
